...
* Thin cluster behind Apache 2.2, configured according to
...
* "god":http://god.rubyforge.org/ configured to monitor apache or nginx, thin, mysql, memcached, drive space and system load
...
* <code>cap ec2onrails:server:set_roles</code> Customizes each instance for it's role(s) (as defined in your Capistrano deploy.rb file). Run this after starting or stopping instances. For now this just makes sure that only the appropriate services (Apache, Thin, and/or MySQL) are running. Eventually this will customize settings for the running services also. Note that an instance can have more than one role. If there's only one instance it will have all roles.
